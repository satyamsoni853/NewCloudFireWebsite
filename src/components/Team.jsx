      ...
      <div className="mx-auto w-full max-w-[1340px] px-6 sm:px-8 md:px-14 lg:px-16">
        ...
        <div className="mx-auto mb-12 grid max-w-6xl grid-cols-2 gap-x-4 gap-y-12 sm:grid-cols-2 lg:grid-cols-3 lg:gap-x-12 lg:gap-y-16">
          {teamMembers.map((member, i) => (
            <Link key={i} to={`/team/${member.slug}`} className="group flex flex-col items-center">
              <div
              ...
              >
                ...
                {member.highlight && (
                  <div className="absolute right-1 top-1/2 flex h-7 w-7 -translate-y-1/2 items-center justify-center rounded-full border-2 border-primary bg-white text-primary shadow-md sm:right-0 sm:h-10 sm:w-10">
              ...
              <div className="text-center">
                <h3
                  className={`mb-1 text-base font-bold sm:text-xl ${
                    member.highlight ? 'text-primary' : 'text-gray-900'
                  }`}
                >
                  {member.name}
                </h3>
                <p className="text-[10px] font-medium uppercase tracking-wider text-primary sm:text-sm">
                  {member.role}
                </p>
              </div>
            </Link>
